...
class VisionMLP_sa(nn.Module):
	...
	def forward(self, image_full, image_compress=None, compress_reduce_factor=None, per_crop_token_len=576, attention_mask=None):
		...
		image_full = self.input_proj(image_full)
		# residual = image_full
		# image_full = image_full.view(bs*num_image_crops, side_len_full, side_len_full, -1).permute(0, 3, 1, 2).contiguous()
		# image_full = self.input_conv(image_full).permute(0, 2, 3, 1).contiguous()
		# image_full = residual + image_full.view(bs, num_image_crops*side_len_full*side_len_full, -1)
